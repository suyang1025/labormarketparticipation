...
for i = 1 : 2
  ...
  parfor j = 1:length(wVec)
    ctemp = zeros(length(ssVec), length(ThetaVals));
    ptemp = zeros(length(ssVec), length(ThetaVals));
    ltemp = zeros(length(ssVec), length(ThetaVals));
    ftemp = zeros(length(ssVec), length(ThetaVals));
    ...
    for jj = 1:length(ssVec)
      for k = 1:length(ThetaVals)
        ...
        elseif(t < 45 && t > 40) % t = 45 to 41
          lowc = cvalLifeCopy(j, jj, k, t + 1) - 5;
          highc = cvalLifeCopy(j, jj, k, t + 1) + 5;
        ...
        elseif(t < 45 && wVec(j) > 40)
          lowp = pvalLifeCopy(j, jj, k, t + 1) - 0.2;
          highp = pvalLifeCopy(j, jj, k, t + 1) + 0.2; 
          lowp2 = ntoi(lowp, 1, pgrid, np);
          highp2 = ntoi(highp, 1, pgrid, np);
          pgrid2 = pgrid(1, lowp2 : highp2);
        else
        ...
        vtplusonei = griddedInterpolant(WW, SS, tplusonefval, 'linear', 'nearest'); 
      ...
      savingsGeqzero = max(savings, zeros(size(savings)));
      ...
      for n = 1:length(calRVals)
        ...
        wtplusone = investret.*savingsGeqzero; % wealth in the next period would be zero if current period savings is negative, this would result in a very small next period value function, so the corresponding consumption would never be chosen
      ...
      ftemp(jj, k) = val; % record the values and we are done for one state grid point! :)
      ctemp(jj, k) = CN(index);
      ptemp(jj, k) = PN(index);
      ltemp(jj, k) = LN(index);
      ...
      for n = 1: numel(TShock)
        ...
        vtplusonei = griddedInterpolant(WW, SS,  tplusonefval(:, :, thetaindx, 1), 'linear', 'nearest');
