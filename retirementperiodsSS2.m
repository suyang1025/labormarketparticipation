...
for i = 1:length(ssVec) % loop through each SS value
    ...
    if(ssVec(i) < 0.2) % given the current state (SS value), retirement income is determined
    rety(i, 1) = 0.9*ssVec(i);
    elseif(ssVec(i) < 1 && ssVec(i) >= 0.2 )
     rety(i, 1) = 0.18 + 0.32*ssVec(i);
    else
     rety(i, 1) = 0.5 + 0.15*ssVec(i);
    end
    
%    rety(i,1) = lambda*ssVec(i); % a second naive specification: retirement income is a portion of avg of highest 35 year incomes, no bend pointsinvolved 
