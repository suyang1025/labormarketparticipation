...
cdllaborsupply = zeros(td - tb, 1); % conditional labor market participation rate
...
for i = 1 : 80
   ...
   % We also know today's savings, in this version I am not doing anything
   % if savining is negative 
   savepath(i, j) = wpath(i, j) + earnings - cpath(i, j);
...
%    end
   ...
   wpath(i + 1, j) = investret.*savepath(i,j);
...
%    
   ...
   wealth(i, j) = wpath(i, j)*perminc(i, j);
    ...
    cpath(i, j) = cvalLifei(wpath(i, j));
    cpath(i, j) = max( cpath(i, j), cplowerbound); %make sure consumption is greater than 0
